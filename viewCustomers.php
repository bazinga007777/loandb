...
<body>
  ...
  $sql = "SELECT fname,phoneno,email,type,doc,DATE_FORMAT(date, '%D %b, %Y') as date,formno FROM auth JOIN loanforms ON auth.username = loanforms.username WHERE branch = '$branchname' AND verification= 'Approved'";
  $result = mysqli_query($conn, $sql);
  $num_rows = mysqli_num_rows($result);
  if ($num_rows == 0) {
    echo "
    <div class='container my-auto'>
    <div class='h1 text-center' style='padding-top: 250px'>No Customers Available!</div>
    </div>
    ";
  } else {
    echo '
          ...
          <th scope="col" style="text-align: center">Applicaton Form</th>
          </tr>
          </thead>
          <tbody>
          ';
